export const LENTES = {
  "Trifocal": [
    "Alcon PanOptix (TFNT00)",
    "Alcon PanOptix Toric (TFNT30-T60)",
    "Zeiss AT LISA tri 839MP",
    "Zeiss AT LISA tri Toric 939MP",
    "Carl Zeiss AT TRIEDY",
    "PhysIOL FineVision POD F",
    "PhysIOL FineVision Triumf",
    "Rayner RayOne Trifocal",
    "Medicontur Liberty 677PMY",
    "Hoya Vivinex iSert XY1A"
  ],
  "Full Range": [
    "Johnson & Johnson Tecnis Synergy",
    "Johnson & Johnson Tecnis Synergy Toric",
    "Rayner RayOne Galaxy",
    "Bausch & Lomb LuxSmart",
    "Hanita Intensity",
    "Hanita Intensity SL",
    "Hanita Intensity Toric",
    "Hanita Intensity SL Toric",
    "Hanita SeeLens MMF",
    "Hanita SeeLens MMF Toric"
  ],
  "EDOF (Extended Depth of Focus)": [
    "Johnson & Johnson Tecnis Eyhance",
    "Johnson & Johnson Tecnis Eyhance Toric",
    "Johnson & Johnson Tecnis Symfony",
    "Johnson & Johnson Tecnis Symfony Toric",
    "Alcon Vivity (DFT015)",
    "Alcon Vivity Toric (DFT315-T60)",
    "Bausch & Lomb LuxGood",
    "Zeiss AT LARA 829MP",
    "Zeiss AT LARA Toric 929MP",
    "Rayner RayOne EMV",
    "Hoya Vivinex Impress",
    "PhysIOL MiniWell Ready",
    "Cristalens Artis Symbiose",
    "Asqelio Trifocal EDOF",
    "SAV-IOL SAV Plus"
  ],
  "Plus / Bifocal": [
    "Johnson & Johnson Tecnis Multifocal +2.75",
    "Johnson & Johnson Tecnis Multifocal +3.25",
    "Alcon ReSTOR +2.5",
    "Alcon ReSTOR +3.0",
    "Zeiss AT LISA 809M",
    "Bausch & Lomb Crystalens AO",
    "PhysIOL FineVision POD F GF",
    "Hanita Bi-Flex M"
  ],
  "Monofocal premium": [
    "Alcon AcrySof IQ (SA60AT)",
    "Johnson & Johnson Tecnis 1-piece",
    "Zeiss CT ASPHINA 409MP",
    "Bausch & Lomb enVista",
    "Hoya Vivinex iSert XY1",
    "Hanita Hyperflex",
    "Hanita SeeLens Max",
    "Hanita B-Lens"
  ]
}
