  ...
  return (
    <div style={{ minHeight:'100vh', display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', background:'linear-gradient(160deg, #0c2461 0%, #1e40af 65%, #1d4ed8 100%)', padding:'1.5rem' }}>
      <div style={{ textAlign:'center', color:'white', maxWidth:'380px', width:'100%' }}>
        ...
        <h1 style={{ margin:'0 0 4px', fontSize:'2.2rem', fontWeight:900, letterSpacing:'2px' }}>PROLENS</h1>
        <p style={{ margin:'0 0 4px', fontSize:'1rem', fontWeight:600, opacity:0.92 }}>Curvas de Desenfoque</p>
        <p style={{ margin:'0 0 8px', fontSize:'0.8rem', opacity:0.65 }}>Dr. Leonardo Orjuela · Medellín</p>
        <div style={{ display:'inline-block', background:'rgba(255,255,255,0.18)', borderRadius:'20px', padding:'4px 16px', marginBottom:'1.5rem', fontSize:'0.8rem', fontWeight:700, letterSpacing:'1px' }}>
          MAIdx sd Bench
        </div>

        <div style={{ background:'rgba(255,255,255,0.1)', borderRadius:'14px', padding:'1.25rem', marginBottom:'1.5rem', border:'1px solid rgba(255,255,255,0.18)' }}>
          <p style={{ margin:'0 0 0.75rem', fontSize:'0.88rem', lineHeight:1.65, opacity:0.92 }}>
          ...
          <p style={{ margin:0, fontSize:'0.78rem', opacity:0.65 }}>
            Uso exclusivo para profesionales de salud visual autorizados.
          </p>
        </div>

        <button
          onClick={() => signIn('google', { callbackUrl: '/' })}
          style={{ width:'100%', padding:'1rem', background:'white', color:'#1e293b', border:'none', borderRadius:'12px', fontSize:'1rem', cursor:'pointer', fontWeight:700, marginBottom:'0.75rem', boxShadow:'0 4px 24px rgba(0,0,0,0.25)', display:'flex', alignItems:'center', justifyContent:'center', gap:'12px' }}>
