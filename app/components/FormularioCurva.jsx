...
export default function FormularioCurva({ onMedicionesChange, onGuardado }) {
  ...
  const handleValor = (d, v) => {
    ...
    setValores(nuevos)
    onMedicionesChange(ojo, getMediciones(nuevos[ojo], tipoAV), lentes[ojo])
  }

  const handleOjo = (o) => {
    setOjo(o)
    onMedicionesChange(o, getMediciones(valores[o], tipoAV), lentes[o])
  }

  const handleLente = (o, val) => {
    const nuevos = { ...lentes, [o]: val }
    setLentes(nuevos)
    onMedicionesChange(ojo, getMediciones(valores[ojo], tipoAV), nuevos[ojo])
  }

  const handleTipoAV = (t) => {
    setTipoAV(t)
    onMedicionesChange(ojo, getMediciones(valores[ojo], t), lentes[ojo])
  }
  ...
  const handleGuardar = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/curvas', {
        ...
        body: JSON.stringify({ paciente, documento, fechaNac, ojo, iol: lentes[ojo], refOD, refOI, mediciones })
      })
    ...
    <div style={{ background: 'white', borderRadius: '12px', padding: '1.25rem', boxShadow: '0 1px 4px rgba(0,0,0,0.08)' }}>
      ...
      {['OD','OI','AO'].map(o => (
        <div key={o} style={{ marginBottom: '0.5rem' }}>
          <label style={lbl}>{o === 'OD' ? 'Ojo Derecho (OD)' : o === 'OI' ? 'Ojo Izquierdo (OI)' : 'Ambos (AO)'}</label>
          <select style={inp} value={lentes[o]} onChange={e => handleLente(o, e.target.value)}>
            <option value="">— Sin IOL / No evaluado —</option>
            {Object.entries(LENTES).map(([categoria, lista]) => (
          ...
          {lentes[o] === '__otro__' && (
            <input style={{ ...inp, marginTop: '4px' }} placeholder="Escribir nombre del IOL" onChange={e => handleLente(o, e.target.value === '' ? '__otro__' : e.target.value)} />
          )}
        </div>
      ))}

      <p style={sec}>Ojo a evaluar ahora</p>
      ...
      <p style={{ ...sec, borderTop:'none', paddingTop:0 }}>AV — {ojo} {lentes[ojo] && lentes[ojo] !== '__otro__' ? `· ${lentes[ojo].split('(')[0].trim()}` : ''}</p>
