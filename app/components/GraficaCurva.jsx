...
const categoriaLente = (nombre) => {
  ...
  if (nombre.includes('PanOptix')||nombre.includes('Synergy')||nombre.includes('LISA tri')||nombre.includes('Trifocal')||nombre.includes('FineVision')) return { label:'Trifocal', color:'#7c3aed' }
  if (nombre.includes('EDOF')||nombre.includes('Vivity')||nombre.includes('Eyhance')||nombre.includes('LARA')||nombre.includes('EMV')||nombre.includes('MiniWell')) return { label:'EDOF', color:'#0f766e' }
  if (nombre.includes('+2')||nombre.includes('+3')||nombre.includes('ReSTOR')||nombre.includes('Bifocal')||nombre.includes('Bi-Flex')) return { label:'Bifocal Plus', color:'#d97706' }
  ...
  return (
    <div style={{ background:'white', borderRadius:'12px', padding:'1rem 1.25rem', boxShadow:'0 1px 4px rgba(0,0,0,0.08)' }}>
        ...
        <div>
          ...
          {lente && lente !== '__otro__' && (
            <div style={{ display:'flex', alignItems:'center', gap:6, marginTop:3 }}>
              ...
              {cat && <span style={{ fontSize:'0.7rem', padding:'1px 7px', borderRadius:'10px', background:cat.color+'22', color:cat.color, fontWeight:600 }}>{cat.label}</span>}
            </div>
          )}
        </div>
      ...
      <ResponsiveContainer width="100%" height={230}>
        <LineChart data={datos} margin={{ top:5, right:35, left:10, bottom:45 }}>
          ...
          <YAxis reversed domain={[-0.1, 1.3]} tick={{ fontSize:10 }} width={32}>
