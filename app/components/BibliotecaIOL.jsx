...
export default function BibliotecaIOL({ curvaActual, nombreIOL, ojo, onCerrar }) {
  ...
  useEffect(() => {
    fetch('/api/iol-referencias')
      ...
      .then(d => {
        setIolRefs(d.iol || [])
        // Auto-seleccionar si hay IOL del paciente
        if (nombreIOL) {
  ...
  const getCurvaRef = (iol) => CAMPOS.map(c => iol[c] !== null ? parseFloat(iol[c]) : null)

  const miniSVG = (iol, ancho=120, alto=60) => {
    const curva = getCurvaRef(iol)
...
function DetalleIOL({ iol, curvaActual }) {
  ...
  // Curva del paciente interpolada
  let puntosP = []
